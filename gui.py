...
class LoginWindow(Frame):
    ...
    def login_clicked(self):
        ...
        if check_login(email, hashedPass):
            self.master.destroy()
            self.master = Tk()
...
class CreateAccount(Frame):
    ...
    def create_account(self):
        ...
        if check_email(email) and check_password(password) and check_confirm_pass(pass_confirm, password):
            ...
            hashedPass = hash_password(password)
            loginDatabase.addUser(email, hashedPass)
            self.create_cancel()
        else:
...
class MainWindow(Frame):
    def __init__(self, master, email):
        ...
        self.import_button = Button(self.frame, text='Import Calendar', font='verdana 16 bold', fg='midnight blue',
                                 command=self.import_calendar)
    ...
    def import_calendar(self):
        google_calendar.main(self.email)
        self.master.destroy()
        self.master = Tk()
        self.master.title('OneClick')
        self.master.geometry('1280x720')
...
def hash_password(password):
    ...
    passEnc = bytes(passStr, 'utf-8')
    passHash.update(passEnc)
    passValue = passHash.hexdigest()
    return passValue
